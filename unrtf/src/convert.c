				...
				if (s[0] != '\\')
				{
					...
					if (within_picture)
					{
						...
						if (!pictfile && !nopict_mode)
						{
							...
							//snprintf(picture_path, 255, "pict%03d.%s", picture_file_number++, ext);
							snprintf(picture_path, 255, "%s\\pict%03d.%s", &folder_path, picture_file_number, ext);

							pictfile = fopen(picture_path, "wb");
							//snprintf(picture_path, 255, "image\\pict%03d.%s", picture_file_number++, ext);
							snprintf(picture_path, 255, "%s.html_image\\pict%03d.%s.jpg", pFileName, picture_file_number, ext);
