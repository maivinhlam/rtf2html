...
char* runmain(char* linkInput) {
	
	FILE *f;
	Word *word;
	char *path = NULL;
	char *env_path_p = NULL;
	
	pFileName = linkInput;
	for (char * pCur = linkInput; *pCur != '\0'; pCur++) {
		if (*pCur == '/' || *pCur == '\\')
			pFileName = pCur + 1;
	}
	...
	snprintf(pathHTML, 255, "%s.html", linkInput);
	FILE *htmlFile = freopen(pathHTML, "w", stdout);
	
	snprintf(folder_path, 255, "%s_image", &pathHTML);
	struct stat st = { 0 };
	...
	nopict_mode = debug_mode = dump_mode = inline_mode = no_remap_mode = quiet = FALSE;
	...
	if (op == NULL) {
		fprintf(stderr, "Initialization error: no output format\n");
		exit(1);
	}
	if (!path_checked && check_dirs() == 0) {
	...
	//path = "C:\\Users\\MaiVinhLam\\Desktop\\test.rtf";
	if (path == NULL) {
		f = stdin;
	}
	else {
